...
function arrayMadness(a, b) {
  return (
    a.reduce((acc, val) => acc + val ** 2, 0) >
    b.reduce((acc, val) => acc + val ** 3, 0)
  );
}
...
function iTri(dist) {
  ...
  if (dist <= 2.4)
    return { Swim: `${(TOTAL_DISTANCE - dist).toFixed(2)} to go!` };
  if (dist <= 114.4)
    return { Bike: `${(TOTAL_DISTANCE - dist).toFixed(2)} to go!` };
  if (dist <= 127)
    return { Run: `${(TOTAL_DISTANCE - dist).toFixed(2)} to go!` };
...
function findMiddle(str) {
  ...
  return +numb.substr(
    Math.floor(numb.length / 2) - 1 + (numb.length % 2),
    numb.length % 2 === 0 ? 2 : 1,
  );
}
